...
A correct fill for the remaining boxes might be __EASE, creating both PLEASE and GREASE. For the remainder of this paper, such a pair of words will be referred to as a *word pair*, sharing a *common substring*[^2] and differing by a *split pair*. Conversely, an incorrect fill might be __IERS, which produces the valid word PLIERS and the invalid word \*GRIERS[^1]. Note that each clued split pair might accept several different fills for the common substring, e.g., __UNGE (PLUNGE, GRUNGE); however, as with crosswords, only one solution will also satisfy all the crossing constraints produced by the rest of the puzzle. 
...
[^2]: Or *fill*: this part of the puzzle is filled in by the player rather than the constructor.